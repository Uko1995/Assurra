package com.uko.eaas.escrow.exception;
...
/**
 * Global exception handler for the Escrow Service.
 * <p>
 * Maps business rule violations and common validation failures to meaningful HTTP
 * statuses so that clients (and the API gateway) receive a consistent response
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<Void>> handleDataIntegrityViolation(
        ...
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(ApiResponse.error("Duplicate transaction: an escrow with this idempotency key or reference already exists"));
    }
